...
rule all:
     input:
        ...
        expand(join(workpath,bigwig_dir,"{name}.mm10.RPGC.bw"),name=samples),
        expand(join(workpath,bigwig_dir,"{name}.mm10.120bp.RPGC.bw"),name=samples),
...
rule trim_CutAndRun:
    ...
    params:
        rname="CR:trim",
        trimmomaticver=config['bin']['FASTPVER'],
        workpath=config['project']['workpath'],
    ...
    shell: """
        ...
        fastp {threads} --in1 {input.file1} --in2 {input.file2} --out1 R1.paired.fastq.gz --out2 R2.paired.fastq.gz --length_required 25 -w {threads} -x -g;
...
rule bam2bigwig_CutAndRun:
     ...
     output:
        bigwig1=join(workpath, bigwig_dir, "{name}.mm10.RPGC.bw"),
        bigwig2=join(workpath, bigwig_dir, "{name}.mm10.120bp.RPGC.bw"),
     params:
        rname="CR:bam2bw",
        deeptoolsver=config['bin']['DEEPTOOLSVER'],
        effgenome=config['references']['EFFGENOME']
     threads:32
     shell:"""
